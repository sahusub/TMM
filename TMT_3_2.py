...
def openAndcopyTemplate():
    ...
    TMMAL = WebDriverWait(driver, 40).until(EC.presence_of_element_located(
        (By.XPATH, r'//*[@title=">: Trial Management Timesheet Activities List"]')))
    TMMAL.click()
    time.sleep(1)
    actions = ''
...
def pasteTemplate():
    TimeReporting = WebDriverWait(driver, 40).until(EC.presence_of_element_located(
        (By.XPATH, r'//*[contains(@title, "Time Reporting")]')))
    TimeReporting.click()
    actions = ''
    actions = ActionChains(driver)
    actions.key_down(Keys.CONTROL)
...
def setPredecessor():
    time.sleep(2)
    Search = WebDriverWait(driver, 15).until(EC.element_to_be_clickable(
        (By.XPATH, '//*[contains(@id, "text-filter-")]')))
    time.sleep(4)
    Search.click()
    Search.send_keys('Study ')
    time.sleep(2)
...
def openAuthDP():
    ...
    # In PROD only >>: Time Report or >>: Time Reporting
    ...
    TimeReport = WebDriverWait(driver, 40).until(EC.presence_of_element_located(
        (By.XPATH, r'//*[@title=">>: Time Report" or @title=">: Time Report" or @title=">>: Time Reporting"]')))
    TimeReport.click()
    ActionChains(driver) \
        .key_down(Keys.SHIFT) \
        .key_down(Keys.F9) \
    ...
    Bookmarks = WebDriverWait(driver, 10).until(EC.presence_of_element_located(
        (By.XPATH, r'//*[@id="pickerTabBar"]/li[3]/a')))
    Bookmarks.click()
    time.sleep(2)
    driver.switch_to_frame('iframeFavoritesView')
    driver.switch_to_frame('frameServerCall')
    CB = WebDriverWait(driver, 10).until(EC.presence_of_element_located(
        (By.ID, 'sel_all')))
    time.sleep(1)
    CB.click()
...
def delete(GDO):
    Schedule = WebDriverWait(driver, 20).until(EC.presence_of_all_elements_located(
        (By.XPATH, '//*[@class = "pvSelectContainer form-field tray-button pivot-select"]')))
    if len(Schedule)>0:
        
        Schedule = WebDriverWait(driver, 20).until(EC.presence_of_element_located(
        (By.XPATH, '//*[@class = "pvSelectContainer form-field tray-button pivot-select"]')))
        time.sleep(1)
        Schedule.click()
        actions = ''
        ...
        GDOElement = WebDriverWait(driver, 10).until(EC.presence_of_all_elements_located(
        ...
        if len(GDOElement)>0:
            ...
            print('Global Development Operations not Present')
